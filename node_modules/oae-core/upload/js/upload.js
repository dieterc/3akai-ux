...
define(['jquery', 'oae.core', 'jquery.fileupload', 'jquery.iframe-transport', 'jquery.jeditable'], function($, oae) {
    ...
    return function(uid, showSettings) {
        ...
        // Keeps track of the viewers of the files
        var viewers = [];
        ...
        var reset = function() {
            ...
            // Reset value of upload-input to upload same files without refreshing
         ...
         */
        var getTypei18n = function(count) {
            if (count > 1) {
                return oae.api.i18n.translate('__MSG__FILE_PLURAL__', 'upload');
            } else {
                return oae.api.i18n.translate('__MSG__FILE_SINGULAR__', 'upload');
            }
        };

        /**
         * Shows a success or failure notification when the uploads have completed.
         *
         * @param  {Number}   errCount   The number of errors that occurred during the file upload
         */
        var showCompleteNotification = function(errCount) {
            ...
            var notification = oae.api.util.template().render($('#upload-notification-template', $rootel), {
                ...
                'type': getTypei18n(selectedFiles.length)
            });
            oae.api.util.notification('', notification.replace(/<(?:.|\n)*?>/gm, ''), errCount ? 'error' : '');
        ...
        var setUpSetPermissionsChanged = function() {
            $(document).on('oae.setpermissions.changed', function(ev, data) {
                ...
                // Assign the original array to viewers so we can reuse it later for notifications
